...
def con_process(filepath, cmd_format):
    try:
        with open(filepath, "r") as fp:
            xml_data = fp.read()
        dict_data = xmltodict.parse(xml_data)
        if cmd_format == 'file':
            symid = dict_data['SymCLI_ML']['Backup']['Backup_Info'][0]['symid']
            dict_data2 = dict_data['SymCLI_ML']['Backup']
        else:
            symid = dict_data['SymCLI_ML']['Symmetrix']['Symm_Info']['symid']
            dict_data2 = dict_data['SymCLI_ML']['Symmetrix']
    except:
        ...
        if cmd_format == 'file':
            df_dev = pd.DataFrame(index=[], columns=['MvName', 'IgName', 'Symdev', 'HostLun'])
            df_dev = df_dev.astype({'HostLun': str})
        else:
        ...
        t01 = []
        for i01 in range(len(dict_data2['Masking_View'])):
            #print("{}/{}".format(i01, len(dict_data2['Masking_View'])))
            flg01 = False
            if dict_data2['Masking_View'][i01]['View_Info']['Initiators'] != None:
                for k01 in dict_data2['Masking_View'][i01]['View_Info']['Initiators']:
                    if k01 == 'group_name':
                        flg01 = True
                        break
            if not flg01:
                df01 = pd.DataFrame.from_dict(dict_data2['Masking_View'][i01])
                df01 = df01.T
                df01 = df01.reset_index(drop=True)
                if cmd_format == 'file':
                    df01 = df01.drop(columns=['Device', 'Initiator_List', 'Initiators', 'SG_Child_info', 'last_updated', 'port_info'])
                else:
                ...
                if dict_data2['Masking_View'][i01]['View_Info']['Initiators'] == None:
                    ...
                    if type(dict_data2['Masking_View'][i01]['View_Info']['Initiators']['wwn']) == str or type(dict_data2['Masking_View'][i01]['View_Info']['Initiators']['wwn']) == dict:
                        df02 = pd.DataFrame(dict_data2['Masking_View'][i01]['View_Info']['Initiators'], index=[0], dtype=str)
                    else:
                        df02 = pd.DataFrame(dict_data2['Masking_View'][i01]['View_Info']['Initiators'], dtype=str)
                if type(dict_data2['Masking_View'][i01]['View_Info']['port_info']['Director_Identification']) == str or type(dict_data2['Masking_View'][i01]['View_Info']['port_info']['Director_Identification']) == dict:
                    df03 = pd.DataFrame([dict_data2['Masking_View'][i01]['View_Info']['port_info']['Director_Identification']], dtype=str)
                else:
                    df03 = pd.DataFrame(dict_data2['Masking_View'][i01]['View_Info']['port_info']['Director_Identification'], dtype=str)

                if dict_data2['Masking_View'][i01]['View_Info']['SG_Child_info']['child_count'] == '0':
                    df04 = pd.DataFrame([['', '']], columns=['group_name', 'Status'])
                else:
                    if type(dict_data2['Masking_View'][i01]['View_Info']['SG_Child_info']['SG']) == str or type(dict_data2['Masking_View'][i01]['View_Info']['SG_Child_info']['SG']) == dict:
                        df04 = pd.DataFrame([dict_data2['Masking_View'][i01]['View_Info']['SG_Child_info']['SG']], dtype=str)
                    else:
                        df04 = pd.DataFrame(dict_data2['Masking_View'][i01]['View_Info']['SG_Child_info']['SG'], dtype=str)
                t03 = []
                if type(dict_data2['Masking_View'][i01]['View_Info']['Device']) == str or type(dict_data2['Masking_View'][i01]['View_Info']['Device']) == dict:
                    t04 = [dict_data2['Masking_View'][i01]['View_Info']['Device']]
                else:
                    t04 = dict_data2['Masking_View'][i01]['View_Info']['Device']
                for i02 in range(len(t04)):
                    df05 = pd.DataFrame(t04[i02], dtype=str)
                    if cmd_format == 'file':
                        t02 = [df05['dev_name'].unique()[0], df05['host_lun'].unique()[0]]
                    else:
                        t02 = [df05['dev_name'].unique()[0], int(df05['capacity'].unique()[0]), int(df05['capacity'].unique()[0]) / 1024, df05['host_lun'].unique()[0]]
                    t03.append(t02)
                if cmd_format == 'file':
                    df06 = pd.DataFrame(t03, columns=['Symdev', 'HostLun'])
                else:
        ...
        ################################
        if cmd_format == 'file':
            df_mv = pd.DataFrame(index=[], columns=['MvName', 'IgName', 'Hba', 'HbaWwpn', 'Alias', 'CascadedIgName', 'PgName', 'FaDir', 'FaPort', 'Wwpn', 'SgName', 'CascadedSgName', 'Symdev', 'HostLun'])
            df_mv = df_mv.astype({'HostLun': str})
        else:
        ...
        for i01 in range(len(dict_data2['Masking_View'])):
            ...
            if dict_data2['Masking_View'][i01]['View_Info']['Initiators'] != None:
                for k01 in dict_data2['Masking_View'][i01]['View_Info']['Initiators']:
                    if k01 == 'group_name':
                        ...
                        break
            for i02 in t01:
                if i02.split('@')[0] == dict_data2['Masking_View'][i01]['View_Info']['view_name']:
                    flg02 = True
                    break
            ...
            if not flg01 and flg02:
                ...
                if cmd_format == 'file':
                    df05 = df_dev[['Symdev', 'HostLun']][(df_dev['MvName'] == view_name) & (df_dev['IgName'] == ig_name)]
                else:
            ...
            if flg01 and not flg02:
                ...
                view_name = df01['view_name'].values[0]
                if type(dict_data2['Masking_View'][i01]['View_Info']['Initiators']['group_name']) == str or type(dict_data2['Masking_View'][i01]['View_Info']['Initiators']['group_name']) == dict:
                    df02 = pd.DataFrame([dict_data2['Masking_View'][i01]['View_Info']['Initiators']])
                else:
                    df02 = pd.DataFrame(dict_data2['Masking_View'][i01]['View_Info']['Initiators'], dtype=str)
                ig_name = df02.iloc[0, 0]
                if type(dict_data2['Masking_View'][i01]['View_Info']['port_info']['Director_Identification']) == str or type(dict_data2['Masking_View'][i01]['View_Info']['port_info']['Director_Identification']) == dict:
                    df03 = pd.DataFrame([dict_data2['Masking_View'][i01]['View_Info']['port_info']['Director_Identification']])
                else:
                    df03 = pd.DataFrame(dict_data2['Masking_View'][i01]['View_Info']['port_info']['Director_Identification'], dtype=str)
                if dict_data2['Masking_View'][i01]['View_Info']['SG_Child_info']['child_count'] == '0':
                    df04 = pd.DataFrame([['', '']], columns=['group_name', 'Status'])
                    ...
                    if type(dict_data2['Masking_View'][i01]['View_Info']['SG_Child_info']['SG']) == str or type(dict_data2['Masking_View'][i01]['View_Info']['SG_Child_info']['SG']) == dict:
                        df04 = pd.DataFrame([dict_data2['Masking_View'][i01]['View_Info']['SG_Child_info']['SG']])
                    else:
                        df04 = pd.DataFrame(dict_data2['Masking_View'][i01]['View_Info']['SG_Child_info']['SG'], dtype=str)
                df05 = pd.concat(
                    ...
                    axis=1)
                if cmd_format == 'file':
                    df06 = df_dev[['Symdev', 'HostLun']][(df_dev['MvName'] == view_name) & (df_dev['IgName'] == "{} *".format(ig_name))]
                    df06 = df06.reset_index(drop=True)
                    df07 = pd.concat([df05, df06], axis=1)
                    df07 = df07.set_axis(['MvName', 'IgName', 'CascadedIgName', 'PgName', 'FaDir', 'FaPort', 'Wwpn', 'SgName', 'CascadedSgName', 'Symdev', 'HostLun'], axis='columns')
                    ...
                    df_mv = pd.concat([df_mv, df07])
        if cmd_format == 'file':
            df_mv = df_mv.fillna({'MvName': '', 'IgName': '', 'Hba': '', 'HbaWwpn': '', 'Alias': '', 'CascadedIgName': '', 'PgName': '', 'FaDir': '', 'FaPort': '', 'Wwpn': '', 'SgName': '', 'CascadedSgName': '', 'Symdev': '', 'HostLun': ''})
            df_mv = df_mv.astype({'HostLun': str})
        else:
...
def main():
    layout = [
        ...
        [sg.Text("symaccess command format"), sg.Listbox(['symaccess -sid <SYMID> list view -detail', 'symaccess -file <ACL_BLKFILE> list view -detail'], size=(40, 2), key="cmd_format")],
    ...
    while True:
        ...
        if event == 'Create excel file':
            if values['cmd_format'][0] == 'symaccess -sid <SYMID> list view -detail':
                cmd_format = "symid"
            else:
                cmd_format = "file"
            r = con_process(values['file'], cmd_format)
            sg.popup(r)
